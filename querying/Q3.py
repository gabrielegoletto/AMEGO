...
for _, question in tqdm(Q3.iterrows(), total=len(Q3)):
    ...
    for ans in question.answers:
        ...
        answer = answer[obj]
        all_features = [torch.tensor(feat) for feat in noun_bb_feat[video][obj]]
        stacked_features = torch.stack(all_features, dim=0)
        average_features = torch.mean(stacked_features, dim=0, keepdim=True).squeeze(1)
...
with open(f'Results_AMEGO/Q3.json', 'w') as f:
    json.dump(results, f)
print(f"Average: {correctly_answered}/{len(Q3)}")    